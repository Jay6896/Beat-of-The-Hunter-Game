...
var state = "INTRO" # States: INTRO, HIDDEN, TYPING, WAITING
@export var next_scene_path: String = "res://Scenes/placeholder_level.tscn"

# --- NEW: Exported text variables for the Inspector! ---
...
func _ready():
	...
	# 2. Fade to transparent over 1.5 seconds
	var fade_in_tween = create_tween()
	fade_in_tween.tween_property(fade_rect, "modulate:a", 0.0, 1.5)
	
	# 3. Setup the Intro Text (This will show up over the fading screen!)
	textbox.visible = true
	label.visible_ratio = 1.0 
	label.text = intro_text
	
	# 4. Hide the fade rect once the animation is completely done
	await fade_in_tween.finished
	fade_rect.visible = false
	
func _input(event):
	var action_pressed = event.is_action_pressed("ui_left") or event.is_action_pressed("ui_right") or event.is_action_pressed("ui_accept") or event.is_action_pressed("Attack")
...
func _on_dog_enemy_boss_defeated() -> void:
	# --- CHANGED: Now triggers using the variable from the Inspector ---
	trigger_boss_text(boss_defeated_text)

func _on_rhythm_snake_enemy_boss_defeated() -> void:
	_on_dog_enemy_boss_defeated()
